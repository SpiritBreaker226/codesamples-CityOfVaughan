...
var featuredcontentglider={
	...
	init:function(config){
		//jQuery(document).ready(function($){
			var tagGlideTogglers = getDocID(config.togglerid);//holds homepage glide togglers
			
			//checks if there is a divGlideTogglers on the page if so then
			//find each glidecontent class on the page as this is need to count the number of glide togglers to create
			//in order to display them properly
			if(tagGlideTogglers != null)
			{
				var intTogglersCount = classToggleLayerCounting(getDocID(config.gliderid),config.contentclass,'div');//holds the number of glide content in the divBanner
								
				//goes around creating a glide togglers for each glidecontent on the page
				for(var intIndex = 0;intIndex < intTogglersCount;intIndex++)
				{
					tagGlideTogglers.innerHTML += "<div class='toc'><a shape='rect' href='#'>&nbsp;</a></div>";
				}//end of for loop
			}//end of if
